...
void singleEncrypt(const char *plainText, const char *sKey); //printf
void encrypt(const char *plainText, const char *sKey, int num);
...
void encrypt(const char *plainText, const char *sKey, int num)
{
	...
	uint64_t key_right = (uint64_t)strtoull(skey_right, NULL, 16);

	for(int k = 0 ; k < (int)pow(2, num) ; k++) 
	{	
	// generateRoundKeys()
		// K1...K32
		for(int i = 0 ; i < 32 ; i++)
		{
			...
			int rCounter = (i + 1) & 0x1f;
			key_left ^= rCounter >> 1;
			key_right ^= (rCounter & 0x1)  << 15;		
		}
	...
	// PLayer()		
		...
		state ^= roundKeyList[31];  //cipherText
	}
}

// char *encrypt(const char *plainText, const char *sKey)
void singleEncrypt(const char *plainText, const char *sKey)
{
	printf("Plaintext: %s\n", plainText);
	printf("Key: %s (80-bit)\n", sKey);
...
int main()
{
	int pri = 0;	
start:
	printf("Enter prime number for encryption: (e.g. 23)\n");
    scanf("%d", &pri);
	
	if(pri == 0)
	{
		singleEncrypt(pText, key);
	}
	else if(pri > 0)
	{
		clock_t start = clock(), diff;
		
		encrypt(pText, key, pri);
